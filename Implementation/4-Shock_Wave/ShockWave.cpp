...
const double dx = 0.4;
const double dt = 0.1;
const double Tmax = 7200.*dt; //Time steps
...
void initial_conditions(Matrix & u)
{
  ...
  for ( ii = 0;  ii < NX;  ii++ ){
    ...
    u[ii][jj] = 0.4 * ( 1. - ( tanh( (3./12.) * dx*ii - 3.))*( tanh( (3./12.)*dx*ii - 3.) ));// +  0.15 * ( 1. - ( tanh( (4.5/26.) * dx*ii-4.5))*( tanh( (4.5/26.)*dx*ii - 4.5) ));
  }
...
void End_Points(Matrix & u){
  ...
  for(int jj = 1; jj < NT; jj++){
    //u[0][jj]   = 0.;
    u[0][jj]   = 0.4 * ( 1. - ( tanh(- 3.))*( tanh( - 3.) )); // +  0.15 * ( 1. - ( tanh(-4.5))*( tanh( - 4.5) ));
    //u[NX-1][jj] = 0.;
    u[NX-1][jj] = 0.4 * ( 1. - ( tanh( (3./12.) * dx*(NX-1) - 3.))*( tanh( (3./12.)*dx*(NX-1) - 3.) )); // +  0.15* ( 1. - ( tanh( (4.5/26.) * dx*(NX-1)-4.5))*( tanh( (4.5/26.)*dx*(NX-1) - 4.5) ));
  }
}
...
void init_gnuplot(void)
{
  ...
  std::cout << "set out 'KDV.gif'" << std::endl;
  std::cout << "unset key" << std::endl;
  std::cout << "set yrange [-0.1:0.8]" << std::endl;
